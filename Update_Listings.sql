...
CREATE DEFINER=`g`@`192.168.0.3` PROCEDURE `Update_Listings`(
        IN `project` VARCHAR(255)
)
COMMENT 
'This procedure updates the Projects Listing Category. To be run @ 23:00 UTC Daily'
...
SET was = (SELECT `Work Availability Score (W.A.S)`
    FROM grc_listings.`Projects_Main`
    WHERE `Project ID` = project);
    
SET zcd = (SELECT `Zero Credit Days (Z.C.D)`
    FROM grc_listings.`Projects_Main`
    WHERE `Project ID` = project);
...
ELSEIF 
    ((was < '0.1') OR (zcd > '7') OR ((compute < '2500') OR (compute = '0')) OR (vote != 'Out')) AND
    (currentstatus != 'Unlisted') 
